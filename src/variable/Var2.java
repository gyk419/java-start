package variable;

public class Var2 {
    public static void main(String[] args) {
        //정수
        byte b = 127; // -128 ~ 127 (1byte,  2⁸)
        short s = 32767; // -32,768 ~ 32,767 (2byte,  2¹⁶)
        int i = 2147483647; // -2,147,483,648 ~ 2,147,483,647 (약 20억) (4byte,  2³)
        long l = 9223372036854775807L; //-9,223,372,036,854,775,808 ~ 9,223,372,036,854,775,807 (8byte,  2⁶⁴)

        //실수
        float f = 10.0f; // 대략 -3.4E38 ~ 3.4E38, 7자리 정밀도 (4byte, 2³²)
        double d = 10.0; // 대략 -1.7E308 ~ 1.7E308, 15자리 정밀도 (8byte, 2⁶⁴)

        //기타
        boolean bool = true; // (1byte)
        char c = 'a'; // (1byte)
        String S = "hello"; // 메모리 사용량은 문자 길이에 따라 동적으로 달라진다. (특별한 타입, 자세한 내용은 뒤에서 학습)
    }
}
